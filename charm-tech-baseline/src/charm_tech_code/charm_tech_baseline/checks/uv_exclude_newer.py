...
def main() -> int:
    ...
    if not have_parser:
        ...
        if re.search(r'^\s*exclude-newer\s*=', text, re.MULTILINE) and re.search(
            r'^\[tool\.uv\]', text, re.MULTILINE
        ):
            emit_check(
                CHECK_ID,
                'pass',
                (
                    'exclude-newer present in pyproject.toml (value not validated — '
                    'python3+tomllib '
                    'unavailable).'
                ),
        ...
        emit_check(
            ...
            {'parser': False, 'exclude_newer_present': False},
            {
                'kind': 'judgement',
                'human_review': (
                    'Add `exclude-newer = "7 days"` under [tool.uv] in pyproject.toml. Prefer a '
                    'friendly-duration string (rolling window) over an RFC 3339 timestamp '
                    '(absolute '
                    'snapshot). See Canonical Security "How-To: Secure a repo" — Minimum release '
                    'age.'
                ),
            },
        )
    ...
    if tool_uv is None:
        ...
        if Path('uv.lock').is_file():
            emit_check(
                CHECK_ID,
                'fail',
                f'uv.lock present but pyproject.toml has no [tool.uv] section. Add [tool.uv] '
                f'with exclude-newer = "{MIN_DAYS} days".',
                {'parser': True, 'tool_uv': False, 'uv_lock_present': True},
                {
                    'kind': 'judgement',
                    'human_review': (
                        'Add [tool.uv] to pyproject.toml with `exclude-newer = "7 days"`. This '
                        'gives every uv resolution path (manual uv add, uv lock regens, uvx '
                        'bootstraps, CI re-resolves) a rolling 7-day quarantine on fresh '
                        'releases — '
                        'complementing the Dependabot cooldown that only covers '
                        'Dependabot-authored '
                        'PRs. See Canonical Security "How-To: Secure a repo" — Minimum release '
                        'age.'
                    ),
                },
            )
        ...
        emit_check(
            ...
            'na',
            (
                'pyproject.toml has no [tool.uv] section and no uv.lock — not a uv-configured '
                'project.'
            ),
    ...
    if 'exclude-newer' not in tool_uv:
        emit_check(
            CHECK_ID,
            'fail',
            f'[tool.uv] present but exclude-newer not set. Add exclude-newer = "{MIN_DAYS} '
            f'days" to give every uv resolution path (manual uv add, uv lock, uvx, CI '
            f're-resolves) a rolling {MIN_DAYS}-day quarantine on fresh releases.',
    ...
    if kind == 'snapshot':
        emit_check(
            CHECK_ID,
            'pass',
            f'exclude-newer set to an RFC 3339 timestamp (absolute snapshot). Accepted but '
            f'note: this freezes resolution to a moment and drifts silently as time passes; '
            f'prefer a rolling friendly-duration like "{MIN_DAYS} days".',
            evidence,
        )
        return EXIT_PASS
    ...
    if kind in ('iso8601', 'friendly'):
        ...
        if days_int >= MIN_DAYS:
            emit_check(
                CHECK_ID,
                'pass',
                f"exclude-newer = '{value}' ({kind}, ≈{days} days). Rolling ≥{MIN_DAYS}-day "
                f'quarantine.',
                evidence,
            )
            return EXIT_PASS
        emit_check(
            CHECK_ID,
            'fail',
            f"exclude-newer = '{value}' ({kind}, ≈{days} days) is below the {MIN_DAYS}-day "
            f'baseline.',
            evidence,
            {
                'kind': 'judgement',
                'human_review': (
                    'Widen exclude-newer to at least "7 days" to match the Charm Tech baseline '
                    'and '
                    'the existing Dependabot cooldown.'
                ),
            },
        )
        return EXIT_FAIL
    ...
    emit_check(
        CHECK_ID,
        'fail',
        f"exclude-newer = '{value}' did not parse as an RFC 3339 timestamp, ISO 8601 duration, "
        f'or friendly duration.',
        evidence,
        {
            'kind': 'judgement',
            'human_review': 'Set exclude-newer to a friendly duration like "7 days" per the uv '
            'docs.',
        },
    )
    return EXIT_FAIL
