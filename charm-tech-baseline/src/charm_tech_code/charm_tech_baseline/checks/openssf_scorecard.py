...
def main() -> int:
    """Check for the Scorecard workflow and README badge, and return the exit code."""
    tier = parse_tier()
    ...
    if not workflow and not badge:
        emit_check(
            ...
            'fail',
            (
                'No OpenSSF Scorecard workflow or badge. (Note: 26.10-cycle rollout is gated '
                'behind '
                "operator's adoption — see references/open-investigations.md.)"
            ),
            {},
            {
                'kind': 'judgement',
                'human_review': (
                    'Wait for operator adoption to settle and propagate its workflow + '
                    'branch-protection wiring; do not invent conventions ahead of it.'
                ),
            },
        )
        return EXIT_FAIL
    ...
    emit_check(
        ...
        {
            'kind': 'judgement',
            'human_review': 'Add the missing half (workflow or badge) to match the operator-led '
            'convention.',
        },
    )
    return EXIT_FAIL
