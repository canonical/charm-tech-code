...
def cadence_check_yaml(workflow: str) -> tuple[bool, str]:
    ...
    return (
        False,
        f'workflow triggers ({joined}) include no cadence trigger (release / schedule / push: '
        f'tags)',
    )
...
def cadence_check_grep(workflow: str) -> tuple[bool, str]:
    try:
        ...
        return (
            False,
            (
                'no release/schedule/push-tags trigger found (grep fallback; install '
                'python3+PyYAML '
                'for accurate check)'
            ),
        )
    ...
    return (
        False,
        (
            'no release/schedule/push-tags trigger found (grep fallback; install python3+PyYAML '
            'for '
            'accurate check)'
        ),
    )
...
def main() -> int:
    ...
    if not manifests and not workflow:
        emit_check(
            ...
            {},
            {
                'kind': 'judgement',
                'human_review': (
                    'Request SBOM via sbom-request.canonical.com Web UI or REST API; store in the '
                    'SSDLC Artifacts directory and request review in ~SSDLC. Add a CI step that '
                    'triggers SBOM generation per release if useful.'
                ),
            },
        )
        return EXIT_FAIL
    ...
    emit_check(
        ...
        {'found': found, 'workflow': workflow},
        {
            'kind': 'judgement',
            'human_review': (
                'SBOM must regenerate per release/cycle. Add `on: release: types: [published]` '
                '(preferred for release-cut workflows) or `on: schedule:` (for unreleased '
                'products) '
                'or `on: push: tags: [v*]`. workflow_dispatch alone is not sufficient.'
            ),
        },
    )
    return EXIT_FAIL
