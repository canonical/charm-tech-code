...
def find_matches(pattern: re.Pattern[str], max_hits: int = 5) -> list[str]:
    """Return up to `max_hits` Go or Python files whose text matches `pattern`."""
    hits: list[str] = []
    for ext in ('*.go', '*.py'):
...
def main() -> int:
    """Check for evidence of SEC0045 security event logging, and return the exit code."""
    tier = parse_tier()
    ...
    if named_evidence:
        ...
        emit_check(
            ...
            'pass',
            (
                'Code references SEC0045 / OWASP security event logging by name (but no specific '
                'event-name tokens detected — confirm the 17 OWASP events are covered).'
            ),
    ...
    emit_check(
        ...
        'fail',
        (
            'No SEC0045 security-event logging detected: neither OWASP-named files nor any '
            'event-name tokens '
            '(authn_/authz_/sys_/user_/session_/excessive_use/malicious_/input_validation_). '
            'Confirm applicability per the per-product disposition in references/decisions.md.'
        ),
        {},
        {
            'kind': 'judgement',
            'human_review': (
                'If the product emits user/admin/auth events, implement the OWASP Application '
                'Logging Vocabulary events in JSON (or logfmt) per canonical/operator#1905 / '
                'canonical/concierge#208. The 17 events span authn_*, authz_*, sys_*, user_*, '
                'session_*, plus excessive_use, malicious_*, input_validation_*.'
            ),
        },
    )
    return EXIT_FAIL
