...
def walk_env(env, scope: str, findings: list[tuple[str, str]], path_parts: list[str]) -> None:
    """Append `(scope, dotted.key)` to `findings` for each env value that uses a secret."""
...
def main() -> int:
    """Check workflow secret-handling hygiene, and return the exit code."""
    tier = parse_tier()
    ...
    if total == 0:
        emit_check(
            CHECK_ID,
            'pass',
            f'No workflow-/job-level env: secrets, echo-secret, or secrets: inherit hits across '
    ...
    remediation = {
        'kind': 'judgement',
        'human_review': (
            'Move secrets to step-level env: (never workflow- or job-level). Never '
            'echo/printf/cat '
            'a secret expression — pass via env: and reference $VAR instead. In reusable-workflow '
            'calls, pass named secrets rather than secrets: inherit.'
        ),
    }
