...
"""Check: CONTRIBUTING.md (or equivalent) documents the PR workflow.

The file must be present AND document the PR workflow with a "Pull requests"
anchor.
...
def main() -> int:
    """Check the contributing guide and its anchor, emit the result, and return the exit code."""
    tier = parse_tier()
    ...
    emit_check(
        CHECK_ID,
        'fail',
        f"{found} present but has no 'Pull requests' heading — the validate-pr-title.py \"Read "
        f'more" URL (#pull-requests) will not anchor.',
        {'path': found, 'pull_requests_heading': False},
        {
            'kind': 'judgement',
            'human_review': (
                'Add a `# Pull requests` (or `## Pull requests`) section listing the allowed '
                'Conventional-Commits types (chore, ci, docs, feat, fix, perf, refactor, revert, '
                'test) and the no-scopes rule. See assets/CONTRIBUTING.md.template for the '
                'canonical shape; for pebble-style HACKING.md the anchor can live there instead.'
            ),
        },
    )
    return EXIT_FAIL
