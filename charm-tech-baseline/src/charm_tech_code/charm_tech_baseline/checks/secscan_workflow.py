...
DIRECT_RE = re.compile(
    r'canonical-secscan-client|run-secscan|sbom-secscan|scan-python', re.IGNORECASE
)
...
def main() -> int:
    ...
    if not sbomber_workflow and not direct_workflow:
        emit_check(
            ...
            {},
            {
                'kind': 'judgement',
                'human_review': (
                    'Reference: canonical/sbomber composite action (Charm Tech default), '
                    'canonical/cs-github-actions run-secscan, or canonical/starflow scan-python. '
                    'Use the --batch instance; GitHub private runners are allow-listed.'
                ),
            },
        )
        return EXIT_FAIL
    ...
    if sbomber_workflow:
        ...
        if not manifests:
            emit_check(
                ...
                'fail',
                (
                    'sbomber workflow present but no .sbomber-manifest*.yaml found at repo root '
                    'or '
                    'under .github/.'
                ),
                {'workflow': workflow_hit},
                {
                    'kind': 'judgement',
                    'human_review': (
                        'Add a .sbomber-manifest-<flavour>.yaml describing artifacts, with '
                        'clients.secscan enabled and ssdlc_params per artifact. See '
                        'canonical/sbomber/examples/all/manifest.yaml.'
                    ),
                },
            )
            return EXIT_FAIL
        ...
        if not problems:
            emit_check(
                ...
                'pass',
                (
                    'sbomber workflow present; manifest enables secscan client and carries '
                    'ssdlc_params.'
                ),
                evidence,
            )
        ...
        emit_check(
            ...
            evidence,
            {
                'kind': 'judgement',
                'human_review': (
                    'In the .sbomber-manifest*.yaml, ensure clients.secscan is enabled and every '
                    'artifact declares ssdlc_params (name/version/channel) — these are what the '
                    'SSDLC scan registry indexes.'
                ),
            },
        )
        return EXIT_FAIL
    ...
    emit_check(
        ...
        {'workflow': workflow_hit, 'driver': 'canonical-secscan-client'},
        {
            'kind': 'judgement',
            'human_review': (
                'Pass --ssdlc-product-name, --ssdlc-cycle, --ssdlc-product-channel, '
                '--ssdlc-product-version so results land in the long-term SSDLC scan registry. '
                '(Or '
                'migrate to canonical/sbomber and move identification into the manifest '
                'ssdlc_params blocks.)'
            ),
        },
    )
