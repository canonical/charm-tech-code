...
"""Check: every PyPI publish path uses Trusted Publishing (OIDC).

Trusted Publishing rather than a long-lived API token. Detects:
...
def main() -> int:
    """Check that PyPI publishing uses Trusted Publishing, and return the exit code."""
    tier = parse_tier()
    ...
    if not publish_files and not twine_files:
        emit_check(
            CHECK_ID,
            'na',
            'No PyPI publish workflow detected (no pypa/gh-action-pypi-publish or twine upload).',
        )
        return EXIT_NA
    ...
    if not problems:
        emit_check(
            ...
            (
                ...
                'pypa/gh-action-pypi-publish, no token input).'
            ),
            evidence,
        )
        return EXIT_PASS
