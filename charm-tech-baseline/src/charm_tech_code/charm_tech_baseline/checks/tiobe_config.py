...
"""Check: TIOBE TICS workflow present, with its auth token and linters.

The workflow must be wired with the auth token, and the language-specific
linters TICS needs must be declared as project dependencies.
...
def main() -> int:
    """Check the TICS workflow, auth token, and linters, and return the exit code."""
    tier = parse_tier()
    ...
    if not wf_dir.is_dir():
        emit_check(
            ...
            {},
            {
                'kind': 'judgement',
                'human_review': (
                    'Add a tiobe.yaml workflow per https://canonical-tiobe-docs.canonical.com/ — '
                    'needs the self-hosted tiobe runner and viewer config selection.'
                ),
            },
        )
        return EXIT_FAIL
    ...
    if not hits:
        emit_check(
            ...
            {},
            {
                'kind': 'judgement',
                'human_review': (
                    'Add a tiobe.yaml workflow using the self-hosted tiobe runner and the '
                    'appropriate viewer config (GoProjects for Go; default for Python).'
                ),
            },
        )
        return EXIT_FAIL
    ...
    emit_check(
        ...
        evidence,
        {
            'kind': 'judgement',
            'human_review': (
                'Add secrets.TICSAUTHTOKEN to the workflow env (TICS publishes nothing without '
                'it). '
                'For Python repos, declare flake8 and pylint in a [dependency-groups] block (or '
                'install them in the workflow). For Go repos, add staticcheck via a tools.go '
                'entry, '
                'Makefile target, or workflow install step. Also confirm a Cobertura coverage '
                'artefact is produced before the TICS step runs — that is repo-specific and not '
                'auto-verified here.'
            ),
        },
    )
