...
def main() -> int:
    ...
    for path in workflow_paths:
        ...
        for pjob in publish_jobs:
            ...
            same_job_attest_no_order = [
                a for a in prec['attests'] if a['index'] >= first_publish_idx
            ]
            if same_job_attests:
                if not any(a['has_subject'] for a in same_job_attests):
                    failures.append(
                        f'{path}:{pjob}: attest step has no '
                        f'subject-path/subject-digest/subject-checksums'
                    )
                    workflow_ok = False
                continue
            if same_job_attest_no_order:
                failures.append(
                    f'{path}:{pjob}: attest step runs AFTER the publish step (must be before)'
                )
                workflow_ok = False
                continue
            ...
            if not any(a['has_subject'] for _, a in upstream_attests):
                ...
                failures.append(
                    f'{path}:{pjob}: upstream attest step(s) in {ujs} have no subject-path'
                )
                workflow_ok = False
                continue
        if workflow_ok:
    ...
    if not publish_workflows:
        emit_check(
            CHECK_ID, 'na', 'No publish/release workflow found; attestation not applicable.'
        )
        return EXIT_NA
