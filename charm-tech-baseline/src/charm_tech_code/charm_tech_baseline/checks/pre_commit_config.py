...
def main() -> int:
    """Check for a pre-commit config, and return the exit code."""
    tier = parse_tier()
    ...
    if (
        not Path('.pre-commit-config.yaml').is_file()
        and not Path('.pre-commit-config.yml').is_file()
    ):
        emit_check(
            CHECK_ID,
            'fail',
            ...
            {},
            {
                'kind': 'judgement',
                'human_review': (
                    'Add a minimal config for the languages in use; hooks should use language: '
                    'system to invoke tools from the uv-locked dependency-groups.'
                ),
            },
        )
        return EXIT_FAIL
    ...
    if versioned_revs > 0:
        emit_check(
            CHECK_ID,
            'fail',
            f'Pre-commit config pins {versioned_revs} rev: version(s). Cycle convention is to '
            f'invoke tools via language: system from pyproject.toml [dependency-groups].',
            {'config': config, 'versioned_revs': versioned_revs},
            {
                'kind': 'judgement',
                'human_review': (
                    'Move tool versions to pyproject.toml [dependency-groups]; replace each '
                    'pinned '
                    'hook with a language: system equivalent. Reference: pytest-jubilant#86.'
                ),
            },
        )
        return EXIT_FAIL
    ...
    emit_check(
        ...
        'pass',
        (
            'Pre-commit config present, no tool versions duplicated in rev: fields, and any '
            'surviving rev: is SHA-pinned.'
        ),
        {'config': config},
    )
    return EXIT_PASS
