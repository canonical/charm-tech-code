...
"""Check: GitHub repo settings are declared in CRA, or set manually to the baseline.

Settings are either declared in canonical-repo-automation (CRA) — the
Terraform/Terragrunt control plane that owns repo settings for Charm Tech —
or, for repos not enrolled in CRA, set manually to the baseline.
...
def main() -> int:
    """Check the repo settings against CRA or the baseline, and return the exit code."""
    tier = parse_tier()
    ...
    if tier != 'personal' and owner == 'canonical':
        r = run([
            'gh',
            'api',
            'repos/canonical/canonical-repo-automation',
            '--jq',
            '.default_branch',
        ])
    ...
    if tier != 'personal':
        if allowed_actions in ('selected', 'local_only'):
            ...
            problems.append(
                f"allowed_actions={allowed_actions} (expected 'selected' or 'local_only')"
            )
    ...
    if managed_by_cra and not problems:
        emit_check(
            CHECK_ID,
            'pass',
            f'Settings declared in CRA ({cra_path}); merge/branch-deletion posture matches the '
            f'baseline{unverifiable_note}.',
            evidence,
        )
        return EXIT_PASS
    ...
    if managed_by_cra and problems:
        ...
        emit_check(
            CHECK_ID,
            'fail',
            f'Repo is declared in CRA ({cra_path}) but live settings drift from the baseline: '
            f'{joined}. Run a CRA apply to reconcile; do not patch live settings directly.',
            evidence,
            {
                'kind': 'judgement',
                'human_review': (
                    'Drift between CRA-declared and live settings. Re-apply CRA for the relevant '
                    'group rather than mutating GitHub directly — direct patches will be '
    ...
    if not problems:
        if tier == 'personal':
            emit_check(
                CHECK_ID,
                'pass',
                f'Live settings match the baseline (personal tier — CRA enrolment not '
                f'expected){unverifiable_note}.',
                evidence,
            )
            return EXIT_PASS
        emit_check(
            CHECK_ID,
            'pass',
            f'Live settings match the baseline. Not declared in CRA — confirm whether this repo '
            f'should be enrolled in canonical-repo-automation{unverifiable_note}.',
            evidence,
        )
        return EXIT_PASS
    ...
    emit_check(
        CHECK_ID,
        'fail',
        f'Repo is NOT declared in canonical-repo-automation and live settings drift from '
        f'baseline: {joined}. Either enrol the repo in CRA (preferred for canonical-owned '
        f'repos) or apply the settings manually.',
        evidence,
        {
            'kind': 'judgement',
            'human_review': (
