...
def main() -> int:
    """Check the SEC0030 security documentation coverage, and return the exit code."""
    tier = parse_tier()
    ...
    if not target:
        emit_check(
            ...
            'fail',
            (
                'No security documentation found (docs/explanation/security.md or expanded '
                'SECURITY.md).'
            ),
            {},
            {
                'kind': 'judgement',
                'human_review': (
                    'Either author docs/explanation/security.md (preferred) or expand SECURITY.md '
                    'to cover the seven SEC0030 V1.3 sections.'
                ),
            },
        )
        return EXIT_FAIL
    ...
    emit_check(
        ...
        {'path': target, 'missing': trimmed},
        {
            'kind': 'judgement',
            'human_review': (
                'Add the missing section(s). See operator#2571, pebble#893, jubilant#332, '
                'charm-ubuntu#87 for reference patterns (sentence-case headers, Mermaid diagrams, '
                'bulleted hardening with To-harden intro, channels-bullet-list at end of '
                'Reporting).'
            ),
        },
    )
