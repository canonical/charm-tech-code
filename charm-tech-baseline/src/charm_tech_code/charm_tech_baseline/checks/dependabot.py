...
def main() -> int:
    ...
    if ecos == 0:
        emit_check(
            ...
            {'path': found_path},
            {
                'kind': 'judgement',
                'human_review': (
                    'Add package-ecosystem blocks for each language/runtime the repo uses '
                    '(pip/uv, '
    ...
    if pc_config:
        ...
        if PRECOMMIT_REV_RE.search(pc_text) and not PRECOMMIT_ECO_RE.search(text):
            emit_check(
                CHECK_ID,
                'fail',
                f'{found_path} is missing a pre-commit package-ecosystem entry — required '
                f'because {pc_config} carries rev: entries whose SHAs need Dependabot bumps '
                f'(Charm Tech baseline).',
                {'path': found_path, 'ecosystems': ecos, 'pre_commit_config': pc_config},
                {
                    'kind': 'judgement',
                    'human_review': (
                        'Add a pre-commit package-ecosystem block to dependabot.yaml (same '
                        'shape as '
                        'github-actions, cooldown ≥7 days). See assets/dependabot.yaml.template.'
                    ),
                },
            )
            return EXIT_FAIL
    ...
    try:
        ...
        if not re.search(r'^[ \t]*cooldown:', text, re.MULTILINE):
            emit_check(
                CHECK_ID,
                'fail',
                f'Dependabot configured with {ecos} ecosystem(s), but no cooldown: block found '
                f'and YAML parser errored — could not auto-validate.',
                {'path': found_path, 'ecosystems': ecos, 'cooldown_validated': False},
                {
                    'kind': 'judgement',
                    'human_review': (
                        'Add a cooldown block to every package-ecosystem entry with '
                        'default-days/semver-*-days ≥7.'
                    ),
                },
            )
            return EXIT_FAIL
        emit_check(
            ...
            'unknown',
            (
                'Dependabot present with cooldown block, but YAML parser errored — cooldown '
                'values '
                'not validated.'
            ),
            {'path': found_path, 'ecosystems': ecos, 'cooldown_validated': False},
    ...
    if not problems:
        emit_check(
            CHECK_ID,
            'pass',
            f'Dependabot configured with {ecos} ecosystem(s); cooldown ≥{MIN_COOLDOWN_DAYS} '
            f'days on every entry.',
    ...
    emit_check(
        CHECK_ID,
        'fail',
        f'Dependabot present but cooldown below Charm Tech baseline (≥{MIN_COOLDOWN_DAYS} days) '
        f'on one or more ecosystems.',
        evidence,
        {
            'kind': 'judgement',
            'human_review': (
                'Set cooldown.default-days (and any per-semver-tier overrides) to at least 7 on '
                'every package-ecosystem entry. See assets/dependabot.yaml.template.'
            ),
        },
    )
    return EXIT_FAIL
