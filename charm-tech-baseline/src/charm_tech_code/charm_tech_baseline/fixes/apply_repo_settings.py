...
def main() -> int:
    """Patch the repo settings (or print the patch with --dry-run), and return the exit code."""
    dry_run = False
    ...
    if owner == 'canonical':
        ...
        sys.stdout.write(
            '\nNote: allowed_actions set to "selected". The selected-actions allowlist itself '
            'is org-scoped and lives in canonical-repo-automation; this repo will inherit '
            'whatever the org allows. If the repo needs additional vetted actions, declare them '
    ...
    if owner == 'canonical':
        sys.stdout.write(
            ...
            'is enrolment in canonical-repo-automation — these patches will drift back over '
            'time without a CRA declaration.\n'
        )
    return 0
